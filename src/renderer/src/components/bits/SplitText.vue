...
/**
 * Vue Bits — TextAnimations/SplitText (https://vue-bits.dev/text-animations/split-text)
 * Adapted: `immediate` prop skips ScrollTrigger (this app's headers never scroll),
 * and the base class no longer forces `whitespace-normal` so titles can truncate.
 */
...
const elClass = computed(() =>
  ['split-parent overflow-hidden inline-block', props.className].filter(Boolean).join(' ')
)
...
watch(
  () => [props.text, props.delay, props.duration, props.ease, props.splitType, props.threshold, props.rootMargin],
  () => {
