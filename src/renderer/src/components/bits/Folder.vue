...
/** Vue Bits — Components/Folder (https://vue-bits.dev/components/folder). Unmodified. */
...
const getPaperClasses = (index: number): string => {
  let sizeClasses = ''
...
const getPaperStyle = (index: number): Record<string, string> => {
  const backgroundColor = index === 0 ? paper1.value : index === 1 ? paper2.value : paper3.value
  const baseStyle = { backgroundColor, borderRadius: '10px', transition: 'all 0.3s ease-in-out' }
  if (open.value) return { ...baseStyle, transform: getOpenTransform(index) }
  return baseStyle
}
...
const frontClass = computed(() => {
  ...
  return open.value
    ? `${baseClasses} ${borderRadius} [transform:skew(15deg)_scaleY(0.6)]`
    : `${baseClasses} ${borderRadius} group-hover:[transform:skew(15deg)_scaleY(0.6)]`
})
