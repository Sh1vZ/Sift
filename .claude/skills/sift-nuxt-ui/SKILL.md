...
`UApp` provides the tooltip, overlay, and toaster contexts, and it must stay the outermost
wrapper so portalled overlays are never clipped by app chrome. Do not move it or add a second one.
